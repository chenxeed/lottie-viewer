...
clientsClaim();
console.log('ServiceWorker: Client Claimed');
...
registerRoute(
  ({ url }) => {
    if (url.origin === self.location.origin && url.pathname.endsWith('.json')) {
      console.log('ServiceWorker: Cache JSON file', url);
      return true;
    }
  },
  new CacheFirst({
...
self.addEventListener('fetch', (ev) => {
  ...
  if (validateUrl.test(url) && method === 'POST') {
    console.log('ServiceWorker: Intercept GraphQL API fetch');
...
async function cacheGraphQLResponse(event: FetchEvent) {
  return fetch(event.request.clone())
    ...
    .catch(async (err) => {
      console.error('ServiceWorker: Failed to get the GraphQL API from server', err);
...
async function setCache(request: Request, response: Response) {
  ...
  if (!(body.query as string).startsWith('query')) {
    console.log(`ServiceWorker: Not caching the GraphQL of ${body.query.substring(0, 50)}...}`);
    return;
  }
  console.log(`ServiceWorker: Caching the GraphQL Query of ${body.query.substring(0, 50)}...}`);
